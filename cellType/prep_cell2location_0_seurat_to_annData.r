...
temp_merge_mini_file <- paste0(temp_dir, "merged_mini.rds")
...
if (file.exists(temp_merge_mini_file)) {
  merged_obj <- readRDS(temp_merge_mini_file)
} else {
  ...
  # Remove unwanted metadata columns, keep only specified slots
  keep_slots <- c("orig.ident", "nCount_Spatial.008um", "nFeature_Spatial.008um", "nCount_Spatial", "nFeature_Spatial", "batch")
  for (slot in setdiff(names(merged_obj@meta.data), keep_slots)) {
    merged_obj@meta.data[[slot]] <- NULL
  }
  ...
  feature_names <- rownames(merged_obj)
  keep_features <- !grepl("^[Mm][Tt]-", feature_names)
  merged_obj <- subset(merged_obj, features = feature_names[keep_features])

  # remove scale.data and data slots from the assay
  merged_obj@assays[[ifelse(VisiumHD, "sketch", "Spatial")]]@scale.data <- NULL
  merged_obj@assays[[ifelse(VisiumHD, "sketch", "Spatial")]]@data <- NULL
  ...
  saveRDS(merged_obj, temp_merge_mini_file)
}
